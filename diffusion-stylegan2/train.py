...
def setup_training_loop_kwargs(
    ...
    # Changed: use custom CSV dataset with feature subsampling
    # Import custom dataset class
    from training.custom_dataset import FeatureSubsampledDataset

    # Set up dataset parameters that will be used to create the dataset in training_loop.py
    # Use custom dataset instead of ImageFolderDataset
    args.training_set_kwargs = dnnlib.EasyDict(
        class_name="training.custom_dataset.FeatureSubsampledDataset",  # Path to our custom class
        path=data,  # Directory with CSV files
        file_ext="csv",  # Look for .csv files (not .png/.jpg)
        resolution=64,  # 64x64 spatial grid
        subsamples_per_sample=5,  # Create 5 subsamples from each CSV file
        features_per_subsample=10,  # Each subsample uses 10 random PCs (out of 50)
        method="one_sample",  # Subsampling method
        use_labels=False,  # No labels
        max_size=None,  # Will be set below
        xflip=False,  # Will be set by mirror option
    )

    # Create a temporary dataset to get info (resolution, length, etc.)
    training_set = FeatureSubsampledDataset(
        path=data,
        file_ext="csv",
        resolution=64,
    ...
    # Update the kwargs with actual values from the dataset
    args.training_set_kwargs.resolution = training_set.resolution  # 64
    args.training_set_kwargs.use_labels = training_set.has_labels  # False
    args.training_set_kwargs.max_size = len(training_set)  # 10 (2 files × 5 subsamples)

    desc = training_set.name  # "FeatureSubsampledDataset"
    ...
    cfg_specs = {
        ...
        "cifar": dict(
            ...
            ramp=0.05,
            map=2,
        ),  # I changed ref_gpus from 2 to 4
        "stl10": dict(
            ref_gpus=4,
            kimg=50000,
            mb=64,
    ...
    args.G_kwargs.synthesis_kwargs.num_fp16_res = args.D_kwargs.num_fp16_res = (
        4  # enable mixed-precision training
    )
    args.G_kwargs.synthesis_kwargs.conv_clamp = args.D_kwargs.conv_clamp = (
    ...
    args.diffusion_kwargs = dnnlib.EasyDict(
        class_name="training.diffusion.Diffusion", **diffusion_specs
    )
...
@click.option(
    ...
    type=click.Choice(
        ["auto", "stylegan2", "paper256", "paper512", "paper1024", "cifar", "stl10"]
    ),
)
...
@click.option(
    "--beta_schedule",
    help="Forward diffusion beta schedule (we use linear always)",
    type=str,
    default="linear",
)
@click.option(
    "--beta_start",
    help="Forward diffusion process beta_start",
    type=float,
    default=1e-4,
)
@click.option(
    "--beta_end", help="Forward diffusion process beta_end", type=float, default=2e-2
)
@click.option(
    "--t_min",
    help="Minimum # of timesteps for adaptively modification",
    type=int,
    default=10,
)
@click.option(
    "--t_max",
    help="Maximum # of timesteps for adaptively modification",
    type=int,
    default=1000,
)
@click.option(
    "--noise_sd", help="Diffusion noise standard deviation", type=float, default=0.05
)
@click.option(
    "--ts_dist",
    help="Diffusion t sampling way",
    type=click.Choice(["priority", "uniform"]),
    default="priority",
)
@click.option("--target", help="Discriminator target value", type=float, default=0.6)
@click.option(
    "--ada_kimg",
    help="# kimgs needed to push diffusion to maximum level",
    type=int,
    default=100,
)
...
@click.option(
    "--aug",
    help="Common image augmentation mode",
    type=click.Choice(["no", "ada", "diff"]),
    default="no",
)
@click.option(
    "--ada_maxp",
    help="The maximum value of p if adding ADA augmentation",
    type=float,
    default=0.25,
)
...
@click.option(
    "--exp_id", help="String to include in result dir name", metavar="STR", type=str
)
def main(ctx, outdir, dry_run, **config_kwargs):
    ...
    matching_dirs = [
        re.fullmatch(r"\d{5}" + f"-{run_desc}", x)
        for x in prev_run_dirs
        if re.fullmatch(r"\d{5}" + f"-{run_desc}", x) is not None
    ]
    if len(matching_dirs) > 0:  # expect unique desc, continue in this directory
        assert (
            len(matching_dirs) == 1
        ), f"Multiple directories found for resuming: {matching_dirs}"
